...
def page_cover(pdf):
    ...
    blocks = [
        ...
        (0, "对官方公告四至内的每座轨道站逐一做点面判定：7 座落在总体设计范围内，"
         ...
         9.2, M, "normal", 0.020),
        (0, "等时圈算法", 13, H, "bold", 0.008),
        (0, "站位取真实站点坐标，不取重点区 polygon 形心。"
            "半径 = 分钟 × 75 米/分钟 ÷ 1.35 绕行系数，对应超级街区的实际绕行："
            "5 分钟约 278 米、10 分钟约 556 米、15 分钟约 833 米。"
            "高速与快速路以红线标出；凡被红线穿过的圈层为名义可达、实际需绕行，"
            "正式成果须用实测步行网络做真等时圈，并计入过街延误、围墙断点与无障碍条件。",
         9.2, M, "normal", 0.020),
...
def page_station(pdf, s, idx):
    ...
    for mins, ang in ((5, 90), (10, 205), (15, 325)):
        _, r = B.walk_ring(s, mins)
        rr = r * 1.06  # just outside the ring, not on the stroke
        lp.place(s["x"] + rr * math.cos(math.radians(ang)),
                 s["y"] + rr * math.sin(math.radians(ang)),
                 "%d分钟 %d米" % (mins, round(r)), size=7.0, color=D.C["rail"],
    ...
    left = [
        ...
        (0.0, "站域口径", 10.0, D.C["ink"], "bold", 0.008),
        (0.015, "以出入口为原点按步行可达界定，半径 = 分钟 × 75 米/分 ÷ 1.35 "
                "绕行系数，故本站 5 分钟 %d 米、10 分钟 %d 米、15 分钟 %d 米。"
                % (r5, r10, round(r15)), 8.2, D.C["mute"], "normal", 0.0),
    ]
    right = [
...
def page_phasing(pdf):
    ...
    lp.place((dzs["x"] + kd.centroid.x) / 2, (dzs["y"] + kd.centroid.y) / 2,
             "偏差 %.0f米" % dist_m, size=9.0, weight="bold", color=D.C["warn"])
    ...
    groups = [
        ...
        ("未解决 / 需官方数据", D.C["warn"], [
            ...
            "PROV-KEY-003 大钟寺 polygon 与真实站位偏差约 %.0f 米，须以官方重点区边界复核" % dist_m,
            "官方红线、控规指标、道路红线、权属、市政与文保控制线均未发布",
            "步行网络、过街延误、围墙断点与无障碍条件均无实测底数",
        ]),
    ]
